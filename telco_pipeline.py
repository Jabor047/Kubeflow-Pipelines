...
def find_eng_and_exp_score(exp_path: str, eng_path: str) -> str:
    ...
    eng_blob = bucket.blob("models/engagement_kmeans.pkl")
    exp_blob = bucket.blob("models/experience_kmeans.pkl")
...
def find_satisfaction(input_path: str):
    ...
    storage_client = storage.Client.from_service_account_json("/data/service_account.json")
    bucket = storage_client.bucket("datatonic-mlops")
    blob = bucket.blob("models/satisfaction_kmeans.pkl")
    ...
    print(f"Cluster points for class 0: {satisfaction_kmeans.cluster_centers_[0]}")
    print(f"Cluster points for class 1: {satisfaction_kmeans.cluster_centers_[1]}")
...
def telco_pipeline():
    ...
    kf_serve_op = kf_serve(
        action="apply",
        model_uri="gs://datatonic-mlops/models/satisfaction_kmeans.pkl",
        model_name="satisfaction-kmeans",
        namespace="gkkarobia",
        framework="sklearn",
        watch_timeout="300"
    )
